"""Code copied from https://github.com/anjany/verse/blob/main/utils/data_utilities.py"""
import argparse
import json
...
def save_centroids(ctd_list, out_path, verb=False):
...
def main(args):
    base_fp = Path(args.path)
    rawdata_fp = base_fp / "rawdata"
    derivatives_fp = base_fp / "derivatives"

    volume_postfix, label_postfix = "_ct.nii.gz", "_seg-subreg_ctd.json"

    studies = [x.name for x in list(rawdata_fp.glob("*"))]

    for study in tqdm(studies):
        study_fp = base_fp / study

        if not study_fp.exists():
            study_fp.mkdir()

        volume_fp = list((rawdata_fp / study).glob("*.nii.gz"))
        volume_fp = sorted([x for x in volume_fp if "seg" not in x.name and "new" not in x.name])
        label_fp = sorted(list((derivatives_fp / study).glob("*.json")))

        for v, l in zip(volume_fp, label_fp):

            vol_name, label_name = str(v.name)[:-len(volume_postfix)], str(l.name)[:-len(label_postfix)]
            assert vol_name == label_name

            img_nib = nib.load(v)
            ctd_list = load_centroids(l)
            ...
            nib.save(img_iso, str(study_fp / vol_name) + ".nii.gz")
            save_centroids(ctd_iso, str(study_fp / label_name) + ".json")


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument("--path", type=str, default='../data/verse/training_dataset/',
                        help="Path to the data set folder, should contain a derivatives and a rawdata folder")
